...
export const useSectionHook = (
  ...
  const [currentPage, setCurrentPage] = useState(
    sessionStorage.getItem(`${id}-page`)
      ? Number(sessionStorage.getItem(`${id}-page`))
      : 1,
  );
  const [pageSize, setPageSize] = useState(
    sessionStorage.getItem(`${id}-page-size`)
      ? Number(sessionStorage.getItem(`${id}-page-size`))
      : pageSizeOptions[0],
  );
  ...
  const handleSetIsAscending = (isAscending: boolean) => {
    ...
    sessionStorage.setItem(`${id}-is-ascending`, isAscending.toString());
  };
